...
Samlp.prototype = {
  getSamlRequestParams: function (opts, callback) {
    ...
    if (options.protocolBinding === BINDINGS.HTTP_POST || !options.deflate) {
      ...
      params.SAMLRequest = Buffer.from(SAMLRequest).toString('base64');
  ...
  decodeResponse: function(req) {
    var decoded = Buffer.from(req.body['SAMLResponse'], 'base64').toString(this.default_encoding);
    ...
    if (encoding && encodingMappings[encoding] && encodingMappings[encoding] !== this.default_encoding){
      ...
      decoded = Buffer.from(req.body['SAMLResponse'], 'base64').toString(encodingMappings[encoding]);
    }
